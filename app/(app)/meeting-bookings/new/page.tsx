...
function NewMeetingBookingForm() {
  ...
  // Fetch available rooms for the selected time slot
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-5 max-w-lg">
      <div className="space-y-3">
        <div>
          <label className="block text-xs font-medium text-gray-600 mb-1">Keperluan / Judul *</label>
          <input
            placeholder="cth. Rapat Bulanan, Interview, dll."
            value={form.title}
        ...
        <div>
          ...
          <input type="date" value={form.date}
            onChange={e => setForm({ ...form, date: e.target.value, meetingRoomId: "" })}
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-500"
          />
        </div>
        ...
        <div>
          ...
          {!form.date || !form.startTime || !form.endTime || durationMin <= 0 ? (
            <p className="text-xs text-gray-400 py-2">Pilih tanggal dan waktu dulu untuk melihat ruangan tersedia</p>
          ) : !rooms ? (
              ...
              {rooms.map((r: any) => (
                <label key={r.id}
                  className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${
                    form.meetingRoomId === String(r.id)
